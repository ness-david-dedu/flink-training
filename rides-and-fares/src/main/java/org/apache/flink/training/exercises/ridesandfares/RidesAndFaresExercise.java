...
public class RidesAndFaresExercise {
    ...
    public static class EnrichmentFunction
        ...
        public void open(Configuration config) {
            rideAndFare = getRuntimeContext().getState(new ValueStateDescriptor<>("rideAndFare", RideAndFare.class));
        }

        @Override
        public void flatMap1(TaxiRide ride, Collector<RideAndFare> out) throws Exception {
            if (rideAndFare.value() == null) {
                RideAndFare initialRideAndFare = new RideAndFare(ride, null);
                rideAndFare.update(initialRideAndFare);
            } else {
        ...
        public void flatMap2(TaxiFare fare, Collector<RideAndFare> out) throws Exception {
            if (rideAndFare.value() == null) {
                RideAndFare initialRideAndFare = new RideAndFare(null, fare);
                rideAndFare.update(initialRideAndFare);
            } else {
                RideAndFare rideAndFareForCollect = rideAndFare.value();
